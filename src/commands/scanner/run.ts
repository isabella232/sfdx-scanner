...
export default class Run extends ScannerCommand {
	...
	public async run(): Promise<AnyJson> {
		...
		// if severity-threshold flag is used, we want to make sure the severities are normalized too.
		...
		const target: string[] = (this.flags.target || []) as string[];
		...
		return new RunOutputProcessor({
			...
			violationsCauseException: this.flags['violations-cause-error'] as boolean,
	...
	private gatherEngineOptions(): Map<string, string> {
		...
		if (this.flags.tsconfig && typeof this.flags.tsconfig === 'string') {
	...
	private validateFlags(): void {
		...
		if (this.flags.format && this.flags.outfile) {
			const format: string = this.flags.format as string;
			const derivedFormat = this.deriveFormatFromOutfile();
			// For validation purposes, treat junit as xml.
			const chosenFormat = format == 'junit' ? 'xml' : format;
			if (derivedFormat !== chosenFormat) {
				this.ux.log(messages.getMessage('validations.outfileFormatMismatch', [format, derivedFormat]));
			}
		}
	}
	...
	private deriveFormatFromOutfile(): OUTPUT_FORMAT {
		...
		const outfile: string = this.flags.outfile;
